...
$sql = "SELECT u.*, 
               ...
               d.mmc_number,
               COALESCE(d.staff_number, a.staff_number) AS id_number,
               v.organization_name,
               d.walletAddress -- Sangat penting untuk MetaMask
        FROM users u
...
<head>
    ...
    <style>
        ...
        .notification-bar { padding: 15px; border-radius: 8px; margin-bottom: 20px; color: white; display: flex; align-items: center; gap: 10px; }
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-header">
        <?php 
            // distinguish icon based on role
            if ($user['role'] == 'admin') {
                echo '<i class="fa-solid fa-user-shield"></i>'; //  Admin
            } elseif ($user['role'] == 'verifier') {
                echo '<i class="fa-solid fa-file-signature"></i>'; //  Verifier
            } else {
                echo '<i class="fa-solid fa-user-doctor"></i>'; // Doctor
            }
        ?>
        <span>SEAL</span>
    </div>
...
<div class="main-wrapper" id="mainWrapper">
    ...
    <div class="container">
        ...
        <?php if (isset($_SESSION['msg'])): ?>
            <div class="notification-bar <?php echo ($_SESSION['msg_type'] == 'success') ? 'bg-success' : 'bg-error'; ?>">
                ...
                <form action="update_profile.php" method="POST">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <div class="form-group">
                            ...
                            <input type="text" value="<?php echo $user['staff_number']; ?>" disabled style="background:#eee;">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="mmc_number" value="<?php echo htmlspecialchars($user['mmc_number']); ?>">
                    </div>
                    <?php endif; ?>
...
<script>
    async function connectMetaMask() {
        if (typeof window.ethereum !== 'undefined') {
            try {
                ...
                // Paparkan di input field
                document.getElementById('walletAddress').value = account;

                // AJAX untuk simpan ke database
    ...
    currentPassInput.addEventListener('input', function() {
        if (this.value.length > 0) {
            ...
            .then(data => {
                if (data === 'match') {
                    currentFeedback.textContent = "✓ Verified"; currentFeedback.style.color = "#1a7f4e"; isCurrentValid = true;
                } else {
                    currentFeedback.textContent = "✗ Incorrect Password"; currentFeedback.style.color = "#d9534f"; isCurrentValid = false;
                }
                validateForm();
            });
        }
    ...
    currentPassInput.addEventListener('input', function() {
    const val = this.value; // Jangan trim di JS supaya user boleh guna space dalam PW
    if (val.length > 0) {
        const formData = new URLSearchParams();
        ...
        fetch('check_current_password.php', {
            method: 'POST',
            body: formData // Menggunakan URLSearchParams lebih selamat untuk encoding
        })
        .then(res => res.text())
        .then(data => {
            // Bersihkan data dari sebarang output PHP yang tidak diingini
